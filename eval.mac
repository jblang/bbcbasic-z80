...
EXPR2$:	EX	AF,AF'
	DEC	A
	AND	7
	CALL	PUSH$		;SAVE STRING ON STACK
...
;Result is integer numeric.
;
INSTR:	CALL	EXPR$C		;STRING TO SEARCH
	CALL	PUSH$		;SAVE STRING ON STACK
	CALL	EXPR$		;SUB-STRING
	POP	BC
	LD	HL,0
	...
	JR	Z,INSTR1
	DEC	A
INSTR1:	LD	DE,ACC$		;DE ADDRESSES SUB
	CALL	SEARCH
	POP	DE
...
;
LEFT$:	CALL	EXPR$C
LEFT1:	CALL	PUSH$		;SAVE STRING ON STACK
	CALL	EXPRI
	POP	BC
	CALL	POP$
	CALL	BRAKET
